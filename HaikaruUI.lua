...
local HikariUI = {}
HikariUI.CurrentTab = nil
HikariUI.Settings = {}
HikariUI.Tabs = {}

if _G.HikariUIInstance then
    _G.HikariUIInstance:Destroy()
    _G.HikariUIInstance = nil
end

-- [[ ===== THEME AND CONFIGURATION ===== ]]
local DefaultTheme = {
    Background = Color3.fromHex("#14120F"),
    Secondary = Color3.fromHex("#1F1C17"),
    Accent = Color3.fromHex("#FFB432"),
    AccentSecondary = Color3.fromHex("#EAA52D"),
    Text = Color3.fromHex("#FFFFFF"),
    TextSecondary = Color3.fromHex("#A8A095"),
    Border = Color3.fromHex("#3C362D"),
    Surface = Color3.fromHex("#1A1713"),
    SurfaceVariant = Color3.fromHex("#24211B"),
    Success = Color3.fromHex("#28C940"),
    Warning = Color3.fromHex("#FFBD2E"),
    Error = Color3.fromHex("#FF5F57"),
    Shadow = Color3.fromHex("#000000")
}

local Theme = DefaultTheme
HikariUI.isMinimized = false

-- [[ ===== ICON HANDLING ===== ]]
-- COLE SUA TABELA 'icons' E AS FUNÇÕES 'getIcon' E 'getAssetUri' AQUI.
local function getIcon(name) return { id = 0, imageRectSize = Vector2.new(48, 48), imageRectOffset = Vector2.new(0, 0) } end
local function getAssetUri(id) return "rbxassetid://" .. tostring(id) end

-- [[ ===== MAIN UI CREATION (CreateMain) ===== ]]
function HikariUI:CreateMain(config)
    local settings = {
        Name = config.Name or "HikariUI",
        title = config.title or config.Title or "HikariUI",
        ToggleUI = config.ToggleUI or "K",
        WindowIcon = config.WindowIcon or "bolt",
        WindowHeight = config.WindowHeight or 450,
        WindowWidth = config.WindowWidth or 650,
        Theme = config.Theme or DefaultTheme,
        Config = {
            Enabled = config.Config and config.Config.Enabled or false,
            FolderName = config.Config and config.Config.FolderName or "HikariUI",
            FileName = config.Config and config.Config.FileName or (config.Name or "HikariUI_Config")
        },
        Blur = {
    ...
    local windowSize = UDim2.new(0, settings.WindowWidth, 0, settings.WindowHeight)
    HikariUI.Settings = settings

    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = settings.Name
    ScreenGui.Parent = game:GetService("CoreGui")
    ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    _G.HikariUIInstance = ScreenGui
    ...
    ShadowFrame.Position = UDim2.new(0.5, -windowSize.X.Offset / 2 + 5, 0.5, -windowSize.Y.Offset / 2 + 5)
    ShadowFrame.Size = UDim2.new(0, windowSize.X.Offset, 0, windowSize.Y.Offset)
    ...
    if settings.Blur.Enable then
        ...
        HikariUI.BlurEffect = BlurEffect
    end

    HikariUI.isDraggingEnabled = true
    ...
    local function makeDraggableConditional(frame)
        ...
        frame.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 and HikariUI.isDraggingEnabled then
                dragging = true
                dragStart = input.Position
                startPos = MainFrame.Position
                TweenService:Create(BorderStroke, TweenInfo.new(0.2), { Color = Theme.Accent, Transparency = 0.3 }):Play()
            end
        end)
        ...
        UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                dragging = false
                TweenService:Create(BorderStroke, TweenInfo.new(0.2), { Color = Theme.Border, Transparency = 0.5 }):Play()
            end
        end)
    end

    local restoreButton = nil
    function HikariUI:MinimizeGUI()
        HikariUI.isMinimized = true
    ...
    local function setupToggleKeybind()
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if input.KeyCode.Name == settings.ToggleUI then
                if HikariUI.isMinimized then
                    HikariUI:RestoreGUI()
                else
                    HikariUI:MinimizeGUI()
                end
            end
        end)
    end
    ...
    TitleBar.ZIndex = 5
    makeDraggableConditional(TitleBar)
    
    local iconData = getIcon(settings.WindowIcon)
    ...
    SettingsButton.Position = UDim2.new(1, -35, 0.5, -12)
    SettingsButton.Size = UDim2.new(0, 24, 0, 24)
    local settingsIconData = getIcon("settings")
    SettingsButton.Image = getAssetUri(settingsIconData.id)
    SettingsButton.ImageRectSize = settingsIconData.imageRectSize
    SettingsButton.ImageRectOffset = settingsIconData.imageRectOffset
    SettingsButton.ImageColor3 = Theme.TextSecondary
    ...
    local AnimatedUnderline = Instance.new("Frame")
    AnimatedUnderline.Name = "AnimatedUnderline"
    AnimatedUnderline.Parent = TabContainer
    AnimatedUnderline.BackgroundColor3 = Theme.Accent
    AnimatedUnderline.BorderSizePixel = 0
    AnimatedUnderline.Size = UDim2.new(0, 0, 0, 2)
    AnimatedUnderline.Position = UDim2.new(0, 15, 1, -2)
    AnimatedUnderline.ZIndex = 5

    local UnderlineCorner = Instance.new("UICorner")
    UnderlineCorner.CornerRadius = UDim.new(0, 2)
    UnderlineCorner.Parent = AnimatedUnderline
    ...
    SettingsButton.MouseButton1Click:Connect(function() HikariUI:ShowSettingsTab() end)
    
    SettingsButton.MouseEnter:Connect(function()
        TweenService:Create(SettingsButton, TweenInfo.new(0.2), { ImageColor3 = Theme.Accent }):Play()
    end)
    SettingsButton.MouseLeave:Connect(function()
        if not (HikariUI.CurrentTab == HikariUI.SettingsContent) then
            TweenService:Create(SettingsButton, TweenInfo.new(0.2), { ImageColor3 = Theme.TextSecondary }):Play()
        end
    end)

    HikariUI.MainFrame = MainFrame
    HikariUI.TabContainer = TabContainer
    HikariUI.ContentContainer = ContentContainer
    HikariUI.AnimatedUnderline = AnimatedUnderline
    HikariUI.SettingsButton = SettingsButton

    return ScreenGui, ContentContainer
end

-- [[ ===== TAB SYSTEM ===== ]]
function HikariUI:CreateTab(name, iconName)
    if not HikariUI.TabContainer or not HikariUI.ContentContainer then
        error("Main GUI must be created first!")
    end

    local textBounds = TextService:GetTextSize(name, 14, Enum.Font.GothamBold, Vector2.new(math.huge, 30))
    local tabWidth = textBounds.X + 20

    local TabButton = Instance.new("TextButton")
    TabButton.Name = name .. "_TabButton"
    TabButton.Parent = HikariUI.TabContainer
    ...
    TabContent.Parent = HikariUI.ContentContainer
    ...
    TabContent.ZIndex = 4
    TabContent.CanvasPosition = Vector2.zero
    ...
    HikariUI.Tabs[TabContent] = tabData

    local function selectTab()
        if HikariUI.CurrentTab == TabContent then return end

        if HikariUI.CurrentTab then
             local oldContent = HikariUI.CurrentTab
             oldContent.Visible = false
        end

        for _, tab in pairs(HikariUI.Tabs) do
            TweenService:Create(tab.Button, TweenInfo.new(0.2), { TextColor3 = Theme.TextSecondary }):Play()
        end
        
        if HikariUI.SettingsContent then
            HikariUI.SettingsContent.Visible = false
            TweenService:Create(HikariUI.SettingsButton, TweenInfo.new(0.2), { ImageColor3 = Theme.TextSecondary }):Play()
        end

        TabContent.Visible = true
        HikariUI.CurrentTab = TabContent
        
        TweenService:Create(TabButton, TweenInfo.new(0.2), { TextColor3 = Theme.Text }):Play()

        local newPos = UDim2.new(0, TabButton.AbsolutePosition.X - HikariUI.TabContainer.AbsolutePosition.X, 1, -2)
        local newSize = UDim2.new(0, TabButton.AbsoluteSize.X, 0, 2)
        TweenService:Create(HikariUI.AnimatedUnderline, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            Position = newPos,
            Size = newSize
        }):Play()
        HikariUI.AnimatedUnderline.Visible = true
    end

    TabButton.MouseButton1Click:Connect(selectTab)

    if not HikariUI.CurrentTab then
...
function HikariUI:CreateSettingsTab()
    if not HikariUI.ContentContainer then
        error("Main GUI must be created first!")
    end

    if HikariUI.SettingsContent then return HikariUI.SettingsContent end

    local SettingsContent = Instance.new("ScrollingFrame")
    SettingsContent.Name = "SettingsContent"
    SettingsContent.Parent = HikariUI.ContentContainer
    ...
    HikariUI.SettingsContent = SettingsContent
    return SettingsContent
end

function HikariUI:ShowSettingsTab()
    if not HikariUI.SettingsContent then
        HikariUI:CreateSettingsTab()
        HikariUI:CreateSection({parent = HikariUI.SettingsContent, text = "General Settings"})

        if HikariUI.Settings.Config.Enabled then
            HikariUI:CreateParagraph({
                parent = HikariUI.SettingsContent, 
                text = "Config settings will be auto-loaded when you execute the script. Make sure to save your changes."
            })
            HikariUI:CreateButton({ parent = HikariUI.SettingsContent, text = "Save Settings", callback = function()
                HikariUI:Save()
                HikariUI:CreateNotify({title = "Settings Saved", description = "All settings have been saved successfully!"})
            end})
            HikariUI:CreateButton({parent = HikariUI.SettingsContent, text = "Delete Settings", callback = function()
                HikariUI:Delete()
                HikariUI:CreateNotify({title = "Settings Reset", description = "All settings have been deleted."})
            end})
            HikariUI:CreateDivider({parent = HikariUI.SettingsContent})
        end

        HikariUI:CreateKeyBind({parent = HikariUI.SettingsContent, text = "Toggle UI", default = HikariUI.Settings.ToggleUI, callback = function(key)
            HikariUI.Settings.ToggleUI = key
        end})
        HikariUI:CreateButton({ parent = HikariUI.SettingsContent, text = "Unload UI", callback = function()
            if _G.HikariUIInstance then _G.HikariUIInstance:Destroy() end
        end})
    end
    
    if HikariUI.CurrentTab == HikariUI.SettingsContent then return end

    if HikariUI.CurrentTab then
        HikariUI.CurrentTab.Visible = false
    end
    ...
    HikariUI.SettingsContent.Visible = true
    HikariUI.CurrentTab = HikariUI.SettingsContent
    HikariUI.AnimatedUnderline.Visible = false
    
    TweenService:Create(HikariUI.SettingsButton, TweenInfo.new(0.2), { ImageColor3 = Theme.Accent }):Play()
end

-- [[ ===== COMPONENT: SLIDER ===== ]]
function HikariUI:CreateSlider(config)
    ...
    if flag ~= nil then SliderFrame:SetAttribute("Flag", flag) end
    ...
    local function setValue(newValue, fireCallback)
        ...
        if callback and fireCallback then pcall(callback, value) end
    end
    ...
    task.defer(function() if callback then pcall(callback, default) end end)

    function SliderObject:Set(newValue) setValue(newValue, true) end
    function SliderObject:Get() return value end
    return SliderObject
end

-- [[ ===== COMPONENT: BUTTON ===== ]]
function HikariUI:CreateButton(config)
    local parent = config.parent or config.Parent
    ...
    ButtonFrame.MouseEnter:Connect(function() TweenService:Create(ButtonFrame, TweenInfo.new(0.2), { BackgroundColor3 = Theme.Border }):Play() end)
    ButtonFrame.MouseLeave:Connect(function() TweenService:Create(ButtonFrame, TweenInfo.new(0.2), { BackgroundColor3 = Theme.Secondary }):Play() end)
    ButtonFrame.MouseButton1Click:Connect(function()
        TweenService:Create(ButtonFrame, TweenInfo.new(0.1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { BackgroundColor3 = Theme.AccentSecondary }):Play()
        task.wait(0.1)
        TweenService:Create(ButtonFrame, TweenInfo.new(0.2), { BackgroundColor3 = Theme.Border }):Play()
...
-- [[ ===== COMPONENT: TOGGLE ===== ]]
function HikariUI:CreateToggle(config)
    local parent = config.parent or config.Parent
    ...
    if flag then ToggleFrame:SetAttribute("Flag", flag) end
    ToggleFrame:SetAttribute("Value", default)
    ...
    local function updateToggleVisual(state)
        TweenService:Create(Switch, TweenInfo.new(0.2), { BackgroundColor3 = state and Theme.Accent or Theme.Border }):Play()
    ...
    ToggleButton.MouseButton1Click:Connect(function() ToggleObject:Set(not toggled) end)
...
-- [[ ===== COMPONENT: DROPDOWN ===== ]]
function HikariUI:CreateDropdown(config)
    local parent = config.parent or config.Parent
    local text = config.text or config.Text or "Dropdown"
    local options = config.options or config.Options or {}
    local flag = config.flag or config.Flag or nil
    local default = config.default or config.Default or options[1]
    local callback = config.callback or config.Callback
    ...
    if flag then DropdownFrame:SetAttribute("Flag", flag) end
    DropdownFrame:SetAttribute("Value", tostring(default or ""))
    ...
    DropdownList.Parent = _G.HikariUIInstance
    ...
    SearchBox.Position = UDim2.new(0.5, -65, 0, 10)
    ...
    local function closeDropdown()
        TweenService:Create(DropdownList, TweenInfo.new(0.2), {Size = UDim2.new(0, 150, 0, 0)}):Play()
        task.wait(0.2)
        DropdownList.Visible = false
    end
    ...
    local function createOptionButton(option)
        ...
        OptionButton.MouseEnter:Connect(function() TweenService:Create(OptionButton, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Border}):Play() end)
        OptionButton.MouseLeave:Connect(function() TweenService:Create(OptionButton, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Secondary}):Play() end)
    end
    
    local function filterAndRefresh(query)
        for _, child in ipairs(ScrollFrame:GetChildren()) do
            if child:IsA("TextButton") then child:Destroy() end
        end
        query = string.lower(query)
        for _, opt in ipairs(options) do
            if string.find(string.lower(tostring(opt)), query, 1, true) then
                createOptionButton(opt)
            end
        end
    end
    
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function() filterAndRefresh(SearchBox.Text) end)
    ...
    DropdownButton.MouseButton1Click:Connect(function()
        if DropdownList.Visible then
            ...
            local spaceBelow = workspace.CurrentCamera.ViewportSize.Y - (pos.Y + size.Y)
            local listHeight = math.min(180, ScrollFrame.CanvasSize.Y.Offset + 58, spaceBelow - 20)
            
            DropdownList.Position = UDim2.new(0, pos.X, 0, pos.Y + size.Y + 5)
            DropdownList.Visible = true
            SearchBox:CaptureFocus()
            TweenService:Create(DropdownList, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
    ...
    UserInputService.InputBegan:Connect(function(input)
        if DropdownList.Visible and input.UserInputType == Enum.UserInputType.MouseButton1 then
            if not DropdownList:IsAncestorOf(input.Position) and not DropdownButton:IsAncestorOf(input.Position) then
                closeDropdown()
            end
        end
    end)

    function DropdownObject:Get() return currentValue end
    ...
    function DropdownObject:Add(item) table.insert(options, item); filterAndRefresh(SearchBox.Text) end
    function DropdownObject:Delete(item) 
        local index = table.find(options, item)
        if index then table.remove(options, index); filterAndRefresh(SearchBox.Text) end
    end
    function DropdownObject:List() return options end
...
-- [[ ===== COMPONENT: KEYBIND ===== ]]
function HikariUI:CreateKeyBind(config)
    local parent = config.parent or config.Parent
    ...
    if flag then KeyBindFrame:SetAttribute("Flag", flag) end
    ...
    local function stopListening()
        ...
        TweenService:Create(KeyBindButton, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Border}):Play()
    end
    ...
    KeyBindButton.MouseButton1Click:Connect(function()
        if isListening then
            ...
            isListening = true
            HikariUI.isDraggingEnabled = false
            KeyBindButton.Text = "..."
            TweenService:Create(KeyBindButton, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Accent}):Play()
            ...
            inputConnection = UserInputService.InputBegan:Connect(function(input, gp)
                ...
                if isListening then
                    ...
                    if keyName ~= "Unknown" then
                        ...
                        if callback then pcall(callback, keyName, input, false) end
                        stopListening()
                    end
                end
            end)
    ...
    UserInputService.InputBegan:Connect(function(input, gp)
        if gp or isListening or not ScreenGui.Enabled then return end
        if currentKey and currentKey ~= "None" and getKeyName(input) == currentKey then
            if callback then pcall(callback, currentKey, input, true) end
        end
    end)

    function KeyBindObject:Set(key) currentKey = tostring(key); KeyBindFrame:SetAttribute("Value", currentKey); KeyBindButton.Text = currentKey end
    function KeyBindObject:Get() return currentKey end
    
    task.defer(function() if callback then pcall(callback, default, nil, false) end end)
    return KeyBindObject
end

-- [[ ===== COMPONENT: INPUT ===== ]]
function HikariUI:CreateInput(config)
    local parent = config.parent or config.Parent
    local text = config.text or config.Text or "Input"
    local placeholder = config.placeholder or config.Placeholder or ""
    local flag = config.flag or config.Flag or nil
    local default = config.default or config.Default or ""
    local callback = config.callback or config.Callback
    ...
    if flag then InputFrame:SetAttribute("Flag", flag) end
    InputFrame:SetAttribute("Value", tostring(default))
    ...
    local function onUpdate(enterPressed)
        InputFrame:SetAttribute("Value", TextBox.Text)
        if callback then pcall(callback, TextBox.Text, enterPressed) end
    end
    
    TextBox.FocusLost:Connect(function(enterPressed) 
        onUpdate(enterPressed) 
        TweenService:Create(TextBox, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Border}):Play()
    end)

    TextBox.Focused:Connect(function()
        TweenService:Create(TextBox, TweenInfo.new(0.2), {BackgroundColor3 = Theme.SurfaceVariant}):Play()
    end)

    function InputObject:Set(value) TextBox.Text = tostring(value); onUpdate(false) end
    function InputObject:Get() return TextBox.Text end
    
    task.defer(function() if callback then pcall(callback, default, false) end end)
    return InputObject
end

-- [[ ===== COMPONENT: PARAGRAPH ===== ]]
function HikariUI:CreateParagraph(config)
    local parent = config.parent or config.Parent
...
-- [[ ===== COMPONENT: SECTION ===== ]]
function HikariUI:CreateSection(config)
    local parent = config.parent or config.Parent
...
-- [[ ===== COMPONENT: DIVIDER ===== ]]
function HikariUI:CreateDivider(config)
    ...
    if parent then DividerFrame.Parent = parent end
    return DividerFrame
end

-- [[ ===== NOTIFICATION SYSTEM ===== ]]
function HikariUI:CreateNotify(config)
    local title = config.title or config.Title or "Notification"
    local description = config.description or config.Description or "No description provided"
    local duration = config.duration or config.time or 5

    if not _G.HikariNotificationStack then _G.HikariNotificationStack = {} end
    ...
    DescriptionLabel.Position = UDim2.new(0, 15, 0, 30)
    DescriptionLabel.Size = UDim2.new(1, -20, 1, -35)
    ...
    table.insert(_G.HikariNotificationStack, 1, NotificationFrame)
    
    local function repositionNotifications()
        for i, notif in ipairs(_G.HikariNotificationStack) do
            if notif and notif.Parent then
                local newYPos = screenSize.Y - (20 + (i * (notifHeight + notifSpacing)))
                TweenService:Create(notif, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), 
                    {Position = UDim2.new(1, -notifWidth - 20, 0, newYPos)}):Play()
            end
        end
    end
    
    local function closeNotification()
        local index = table.find(_G.HikariNotificationStack, NotificationFrame)
        if index then table.remove(_G.HikariNotificationStack, index) end
        
        TweenService:Create(NotificationFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), 
            {Position = UDim2.new(1, 10, NotificationFrame.Position.Y.Scale, NotificationFrame.Position.Y.Offset)}
        ):Play()
        task.wait(0.3)
...
function HikariUI:GetFlagValue(flag)
    if not isfile then return nil end
    local folderName = self.Settings.Config.FolderName
    local fileName = self.Settings.Config.FileName
    ...
    if success and data then return data[flag] end
    return nil
end

function HikariUI:Save()
    if not self.Settings.Config.Enabled or not writefile then return end
    local folderName = self.Settings.Config.FolderName
    local fileName = self.Settings.Config.FileName
    local settingsData = {}
    ...
    local function collectAttributes(obj)
        for _, child in ipairs(obj:GetChildren()) do
            ...
            if flag and value ~= nil then settingsData[flag] = value end
            if #child:GetChildren() > 0 then collectAttributes(child) end
        end
    end

    if _G.HikariUIInstance then collectAttributes(_G.HikariUIInstance) end
    if not isfolder(folderName) then makefolder(folderName) end
    writefile(folderName .. "/" .. fileName .. ".json", HttpService:JSONEncode(settingsData))
end

function HikariUI:Load()
    if not self.Settings.Config.Enabled or not isfile then return end
    local folderName = self.Settings.Config.FolderName
    local fileName = self.Settings.Config.FileName
    local configPath = folderName .. "/" .. fileName .. ".json"
    if not isfile(configPath) then return end

    local success, settingsData = pcall(function() return HttpService:JSONDecode(readfile(configPath)) end)
    if not success or not settingsData then return end

    local function applyAttributes(obj)
        for _, child in ipairs(obj:GetChildren()) do
            local flag = child:GetAttribute("Flag")
            if flag and settingsData[flag] ~= nil then
                 child:SetAttribute("Value", settingsData[flag])
            end
            if #child:GetChildren() > 0 then applyAttributes(child) end
        end
    end
    
    if _G.HikariUIInstance then applyAttributes(_G.HikariUIInstance) end
end

function HikariUI:Delete()
    if not self.Settings.Config.Enabled or not delfile then return end
    local folderName = self.Settings.Config.FolderName
    local fileName = self.Settings.Config.FileName
    local configPath = folderName .. "/" .. fileName .. ".json"
